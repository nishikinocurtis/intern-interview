...
定义状态：`dp[i][j]`：以 (i, j) 为右下角的仅含 1 的正方形的最大边长，同时也表示以 (i, j) 为右下角的正方形的数量(即边长分别为 1， 2， ... `dp[i][j]`的正方形各一个)；
状态转移方程：
1). 如果 `matrix[i][j] == 0`, `dp[i][j] = 0`；
1). 如果 `matrix[i][j] == 1`, `dp[i][j] = min(dp[i - 1][j], d[i][j - 1], dp[i - 1][j - 1]) + 1`；
初始条件：
1). 第一行： 如果`matrix[0][i] == 1`, `dp[0][i] = 1`；
2). 第一列： 如果`matrix[i][0] == 1`, `dp[i][0] = 1`；
计算出 `dp[i][j]`，即以 (i, j) 为右下角的正方形的数量，然后把 `dp[i][j]` 全部加起来即可。
...
维护一个 tails[i] 表示长度为 i 的最长上升子序列的尾部元素值。
...
- 如前半部分比后半部分大，直接将前半部分反转输出。
- 如果前半部分比后半部分小或者相等（本身为回文字符），将前半部分进位后反转输出。
  - 注意，如果进位后字符串第一位为 0，说明前半部分字符串全由 9 组成，需特殊对待。

```c++
class Solution {
public:
    string getLargerPalindrome(string str) {
        string ans;
        int len = str.size();
        int flag = 0;
        for (int i = len / 2 - 1; i >= 0; i--) {
            //前半部分比后半部分大，直接将前半部分反转即可
...
- 顺序扫描中序表达式。
  ...
  - 是运算符
    - "(" 直接入栈
    - ")" 将栈中 "(" 前元素依次出栈并输出，"(" 只出栈，不输出
    - 其他符号，按运算符优先级维护一个单调递增的栈
- 扫描完成后，将栈中剩余符号依次输出。

```c++
class Solution {
    map<char, int> level = {{'*', 1},
                            {'/', 1},
                            {'+', 0},
                            {'-', 0},};
public:
    string getReversePolishNotation(string str) {
        stack<char> stack;
        string ans;
        for (int i = 0; i < str.size(); ++i) {
            char ch = str[i];
            if (ch <= '9' && ch >= '0') {
                ...
                //其他字符维护单调递增栈
                while (!stack.empty() && level[stack.top()] >= level[ch] && stack.top() != '(') {
                    ans += stack.top();
                    stack.pop();
                }
                stack.push(ch);
