...
先进行快照读，另一个事务修改记录后，本事务进行了插入或者更新等操作更改了快照版本，于是读到了和第一次不一样的结果，或是在未查询到主键记录时插入造成主键冲突或者主键冲突但依然插入成功。
...
当前版本如果想避免幻读，需要在每次读时加锁，就会对索引记录加上 Next-Key Lock ，阻塞查询区间内其他事务的修改，因此不会有幻读。
...
1. 事务 1 修改索引为 1 的行，事务 2 修改索引为 2 的行，分别加了排他锁
2. 事务 1 想要修改索引为 2 的行，被阻塞，等待 2 的释放。事务 2 想要修改索引为 1 的行，被阻塞，等待 1 的释放。
3. 事务 1 和 2 互相阻塞，形成死锁。
...
覆盖索引：当需要查询的信息在非聚集索引中就可查找到时，就无需再回表查。
